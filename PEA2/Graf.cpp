...
/*int* Graf::TabuTSP(int iterations, int tabuLength) {
	//inicjalizacja
	int i = 0;
	int* bestPath = new int[size + 1];
	int bestPathValue=0;
	int* newPath = new int[size+1];
	int newPathValue=0;
	int moveValue;
	node bestmove;
	int bestMoveValue = 0;
	...
	for (int i = 1; i < size; i++) {
		bestPath[i] = i;
		bestPathValue += graf[bestPath[i - 1]][bestPath[i]];
	}
	bestPath[size] = 0;
	bestPathValue += graf[bestPath[size - 1]][0];
	...
	//główna pętla
	do {
		//odnadowanie najlepszego ruchu
		for (int i = 1; i < size - 1; i++)
			for (int j = i + 1; j < size; j++) {
				if (find(TabuList.begin(), TabuList.end(), node({ i,j })) == TabuList.end()) {
					//wyliczenie wartości ruchu
					moveValue = graf[bestPath[i]][bestPath[i + 1]];
					moveValue += graf[bestPath[i - 1]][bestPath[i]];
					moveValue += graf[bestPath[j]][bestPath[j + 1]];
					moveValue += graf[bestPath[j - 1]][bestPath[j]];
	...
	newPath[bestmove.a] = bestPath[bestmove.b];
	newPath[bestmove.b] = bestPath[bestmove.a];

	//porównanie starej ścieżki z nową i zamiana
	for (int i = 1; i <= size; i++)
		newPathValue += graf[newPath[i - 1]][newPath[i]];
...
/*
int* Graf::TabuChainTSP(int iterations, int tabuLength) {
	//inicjalizacja
	int i = 0;
	int a = 0, b = 0;
	int* bestPath = new int[size + 1];
	...
	do {
		//odnadowanie najlepszego ruchu
		for (int i = 1; i <= size; i++)
			if (find(TabuList.begin(), TabuList.end(), node({ i,b })) != TabuList.end()) {
				if (graf[bestPath[i - 1]][bestPath[i]] > bestMoveValue) {
					a = b;
					b = i;
					moveValue = bestMoveValue;
					bestMoveValue = graf[bestPath[i - 1]][bestPath[i]];
				}
				else if (graf[bestPath[i - 1]][bestPath[i]] > moveValue) {
					a = i;
					moveValue = graf[bestPath[i - 1]][bestPath[i]];
				}
			}
